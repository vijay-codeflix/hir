...
<div id="page-content">
    ...
    <?php
      ...
      if($this->session->flashdata('flash_message'))
      {
        ...
        {
          $msg="<strong>Well done!</strong> Product category detail inserted with success.";
           $style = 'display: block;';           
        }
        else if($this->session->flashdata('flash_message') == 'updated')
        {
          $msg = "<strong>Well done!</strong> Product category detail updated with success.";
          $style = 'display: block;';
        }else if($this->session->flashdata('flash_message') == 'Deleted')
        {
          $msg = "<strong>Well done!</strong> Product category detail deleted with success.";
          $style = 'display: block;';
        }
        else
        {
    ...
    <div class="block full">
        <div class="block-title">
            <h2><strong>Product Categories List</strong></h2>
        </div>
        ...
        <div class="row">
          <div class="col-md-12">
            <div class="block pull-right no-border">
              <?php echo form_open('admin/productcategories/category_add'); ?>
                  <div class="block-section text-right">
                      <button type="submit" class="btn btn-sm btn-primary"><i class="fa fa-plus"></i> Add Product Category</button>
                  </div>
                  <?php //echo form_close(); ?>
            </div>     
          </div>  
        ...
        <div class="table-responsive">
            <table id="example-datatable" class="table table-vcenter table-condensed table-bordered">
                ...
                <tbody>
                    <?php
                    if($product_categoriesview){
                        for($i = 0; $i < count($product_categoriesview); $i++){
                          $id = str_replace('/', '_',rtrim(base64_encode($product_categoriesview[$i]->id), '=')); ?>
                          <tr>
                              <td class="text-center"><?php echo str_pad($i + 1, 2,"0", STR_PAD_LEFT);; ?></td>
                              <td class="text-center"><?= $product_categoriesview[$i]->id; ?></td>
                              <td class="text-center"><?php echo ucfirst($product_categoriesview[$i]->name); ?></td>
                              <td class="text-center"><?php echo $product_categoriesview[$i]->created_at; ?></td>
                              <td class="text-center">
                                  <div class="btn-group">
                                      <a href="<?php echo CURRENT_MODULE.'productcategories/category_edit/'.$id;?>" data-toggle="tooltip" title="Edit" class="btn btn-xs btn-default"><i class="fa fa-pencil"></i></a>
