...
<div id="page-content">
    ...
    <?php
      ...
      if($this->session->flashdata('flash_message'))
      {
        ...
        {
          $msg="<strong>Well done!</strong> dealer category detail inserted with success.";
           $style = 'display: block;';           
        }
        else if($this->session->flashdata('flash_message') == 'updated')
        {
          $msg = "<strong>Well done!</strong> dealer category detail updated with success.";
          $style = 'display: block;';
        }else if($this->session->flashdata('flash_message') == 'Deleted')
        {
          $msg = "<strong>Well done!</strong> dealer category detail deleted with success.";
          $style = 'display: block;';
        }
        else
        {
    ...
    <div class="block full">
        <div class="block-title">
            <h2><strong>Dealer Categories List</strong></h2>
        </div>

        <!-- Uncomment below code for add dealercategories -->
        <div class="row">
          <div class="col-md-12">
            <div class="block pull-right no-border">
              <?php echo form_open('admin/dealercategories/category_add'); ?>
                  <div class="block-section text-right">
                      <button type="submit" class="btn btn-sm btn-primary"><i class="fa fa-plus"></i> Add Dealer Category</button>
                  </div>
                  <?php //echo form_close(); ?>
            </div>     
          </div>  
            ...
            <table id="example-datatable" class="table table-vcenter table-condensed table-bordered">
                <thead>
                    <tr>
                        ...
                        <th class="text-center">Dealer Category Name</th>
                ...
                <tbody>
                    <?php
                    if($dealer_categoriesview){
                        for($i = 0; $i < count($dealer_categoriesview); $i++){
                          $id = str_replace('/', '_',rtrim(base64_encode($dealer_categoriesview[$i]->id), '=')); ?>
                          <tr>
                              <td class="text-center"><?php echo str_pad($i + 1, 2,"0", STR_PAD_LEFT);; ?></td>
                              <td class="text-center"><?php echo ucfirst($dealer_categoriesview[$i]->name); ?></td>
                              <td class="text-center"><?php echo $dealer_categoriesview[$i]->created_at; ?></td>
                              <td class="text-center">
                                  <div class="btn-group">
                                      <a href="<?php echo CURRENT_MODULE.'dealercategories/category_edit/'.$id;?>" data-toggle="tooltip" title="Edit" class="btn btn-xs btn-default"><i class="fa fa-pencil"></i></a>
                                      <!-- Uncomment below code for delete dealer category -->                                     
