	        ...
	        <div class="col-md-7 col-md-offset-2-5">
	            ...
	            <div class="block">
	                ...
	                <div class="block-title">
	                    ...
	                    <h2><strong>Edit State</strong></h2>
	                </div>
	                <!-- END Form Elements Title -->

	                <!-- Basic Form Elements Content -->
	                <form id = "form-validation" class="form-horizontal form-bordered" method="post" action="<?php echo BASE_URL.'admin/states/update'; ?>">
	                	<input type="hidden" name="state_id" value="<?php echo $states_Details[0]->state_id; ?>" >
	                	<div class="form-group">
	                        <label for="country_id" class="col-md-4 control-label">Country Name<span class="text-danger">*</span></label>
	                        <div class="col-md-7">
	                        	<select size="1" name="country_id" class="form-control select-chosen" onchange="getZones(this)" id="county_selection">
	                        		<option value = ''>Select Country</option>
	                        		<?php 
	                        		if($countrylist){
	                        			for($i = 0; $i < count($countrylist); $i++){
	                        				if($states_Details[0]->country_id == $countrylist[$i]->country_id){
	                        					$selected = 'selected';
	                        				}else{
	                        					$selected = 'false';
	                        				}
	                        	...
	                        	<select size="1" name="zone_id" class="form-control select-chosen" id="zone_id">
	                        		...
	                        		if($zonelist){
	                        			for($i = 0; $i < count($zonelist); $i++){
	                        				if($states_Details[0]->zone_id == $zonelist[$i]->zone_id){
	                        					$selected = 'selected';
	                        				}else{
	                        					$selected = '';
	                        				}
                        ...
                        <div class="form-group">
                        	<label for="State" class="col-md-4 control-label">State<span class="text-danger">*</span></label>
	                        <div class="col-md-7">
	                            <input type="text" placeholder="State name" class="form-control" name="state_name" id="state_name" value="<?php if(set_value('state_name')){ echo set_value('state_name'); }else{ echo $states_Details[0]->state_name; } ?>">
						...
						<div class="form-group form-actions">
	                        <div class="col-md-9 col-md-offset-4">
	                            <button class="btn btn-sm btn-primary" type="submit"><i class="fa fa-angle-right"></i> Update State</button>
	                        </div>
	                    </div>
	                </form>
