...
<div id="page-content">
    ...
    <?php
      ...
      if($this->session->flashdata('flash_message'))
      {
        ...
        {
          $msg="<strong>Well done!</strong> state detail inserted with success.";
           $style = 'display: block;';           
        }
        else if($this->session->flashdata('flash_message') == 'updated')
        {
          $msg = "<strong>Well done!</strong> state detail updated with success.";
          $style = 'display: block;';
        }else if($this->session->flashdata('flash_message') == 'Deleted')
        {
          $msg = "<strong>Well done!</strong> state detail deleted with success.";
          $style = 'display: block;';
        }
        else
        {
    ...
    <div class="block full">
        <div class="block-title">
            <h2><strong>States List</strong></h2>
        </div>

        <!-- Uncomment below code for add state -->
        <div class="row">
          <div class="col-md-12">
            <div class="block pull-right no-border">
              <?php echo form_open('admin/states/stateadd'); ?>
                  <div class="block-section text-right">
                      <button type="submit" class="btn btn-sm btn-primary"><i class="fa fa-plus"></i> Add State</button>
                  </div>
                  <?php //echo form_close(); ?>
            </div>     
          </div>  
        ...
        <div class="table-responsive">
            <table id="example-datatable" class="table table-vcenter table-condensed table-bordered">
                ...
                <tbody>
                    <?php
                    if($statesview){
                        for($i = 0; $i < count($statesview); $i++){
                          $id = str_replace('/', '_',rtrim(base64_encode($statesview[$i]->state_id), '=')); ?>
                          <tr>
                              <td class="text-center"><?php echo str_pad($i + 1, 2,"0", STR_PAD_LEFT);; ?></td>
                              <td class="text-center"><?php echo ucfirst($statesview[$i]->country_name); ?></td>
                              <td class="text-center"><?php echo ucfirst($statesview[$i]->zone_name); ?></td>
                              <td class="text-center"><?php echo ucfirst($statesview[$i]->state_name); ?></td>
                              <td class="text-center"><?php echo $statesview[$i]->created_at; ?></td>
                              <td class="text-center">
                                  <div class="btn-group">
                                      <a href="<?php echo CURRENT_MODULE.'states/stateedit/'.$id;?>" data-toggle="tooltip" title="Edit" class="btn btn-xs btn-default"><i class="fa fa-pencil"></i></a>
                                      <!-- Uncomment below code for delete state -->                                     
