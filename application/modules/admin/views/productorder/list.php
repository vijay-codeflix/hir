        ...
        <table id="example-datatable" class="table table-vcenter table-condensed table-bordered">
          <thead>
            <tr>
              ...
              <th class="text-center">Admin Dispatch Date</th>
          ...
          <tbody>
            <?php
            if ($poview) {
              for ($i = 0; $i < count($poview); $i++) {
                $id = str_replace('/', '_', rtrim(base64_encode($poview[$i]['id']), '=')); ?>
                <tr>
                  <td class="text-center"><?php echo str_pad($i + 1, 2, "0", STR_PAD_LEFT); ?></td>
                  <td class="text-center"><?php echo ucfirst($poview[$i]['po_number']); ?></td>
                  <td class="text-center"><?php echo ucfirst($poview[$i]['company_name']); ?></td>
                  <td class="text-center"><?php echo ucfirst($poview[$i]['employee_name']); ?></td>
                  <td class="text-center"><?php echo $poview[$i]['dispatch_date']; ?></td>
                  <td class="text-center"><?php echo $poview[$i]['admin_dispatch_date'] ?? "-"; ?></td>
                  <!--                              <td class="text-center">-->
                  <!--                                  <div class="btn-group">-->
                  <!--                                      <a href="javascript: show_confirm('--><?php //echo BASE_URL; 
                                                                                                  ?><!--', 'view','--><?php //echo $id; 
                                                                                                                      ?><!--')" onclick="$('#viewcities').modal('show');" data-toggle="tooltip" title="View" class="btn btn-xs"><i class="fa fa-eye"></i></a>-->
                  ...
                  <td class="text-center">
                    <div class="btn-group">
                      <a href="<?php echo CURRENT_MODULE . 'productorder/getCurrentPo/' . $id; ?>" data-toggle="tooltip" title="View details" class="btn btn-xs"><i class="fa fa-eye"></i></a>
                    </div>
                  </td>
                </tr>
              <?php } ?>
