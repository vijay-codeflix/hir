...
<div id="page-content">
    ...
    <?php
      ...
      if($this->session->flashdata('flash_message'))
      {
        ...
        {
          $msg="<strong>Well done!</strong> employee grade detail inserted with success.";
           $style = 'display: block;';           
        }
        else if($this->session->flashdata('flash_message') == 'updated')
        {
          $msg = "<strong>Well done!</strong> employee grade detail updated with success.";
          $style = 'display: block;';
        }else if($this->session->flashdata('flash_message') == 'Deleted')
        {
          $msg = "<strong>Well done!</strong> employee grade detail deleted with success.";
          $style = 'display: block;';
        }
        else
        {
    ...
    <div class="block full">
        <div class="block-title">
            <h2><strong>Employee Grade</strong></h2>
        </div>
        <div class="row">
          <div class="col-md-12">
            <div class="block pull-right no-border">
              <?php echo form_open('admin/users/addEmployeeGrade'); ?>
                  <div class="block-section text-right">
                      <button type="submit" class="btn btn-sm btn-primary"><i class="fa fa-plus"></i> Add Employee Grade</button>
                  </div>
                  <?php echo form_close(); ?>
            </div>     
          </div>  
            ...
            <table id="example-datatable" class="table table-vcenter table-condensed table-bordered">
                <thead>
                    <tr>
                        ...
                        <th class="text-center">Employee Grade</th>
                ...
                <tbody>
                    ...
                    if($grades){
                         for($i = 0; $i < count($grades); $i++){
                          $id = str_replace('/', '_',rtrim(base64_encode($grades[$i]->id), '=')); 
                    ?>
                        <tr>
                            <td class="text-center"><?php echo str_pad($i + 1, 2,"0", STR_PAD_LEFT);; ?></td>
                            <td class="text-center"><?php echo ucfirst($grades[$i]->grade); ?></td>
                            <!-- <td class="text-center"><?php //echo date(DEFAULT_DATETIME_FORMAT,strtotime($grades[$i]->created_at)); ?></td> -->
                            <td class="text-center">
                                <div class="btn-group">
                                    <a href="<?php echo CURRENT_MODULE.'users/viewEmployeeGrade/'.$id;?>" data-toggle="tooltip" title="View" class="btn btn-xs btn-default"><i class="fa fa-eye"></i></a>
                                    <a href="<?php echo CURRENT_MODULE.'users/editEmployeeGrade/'.$id;?>" data-toggle="tooltip" title="Edit" class="btn btn-xs btn-default"><i class="fa fa-pencil"></i></a>
