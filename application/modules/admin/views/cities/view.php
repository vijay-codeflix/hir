...
<div id="page-content">
    ...
    <?php
      ...
      if($this->session->flashdata('flash_message'))
      {
        ...
        {
          $msg="<strong>Well done!</strong> city grade detail inserted with success.";
           $style = 'display: block;';           
        }
        else if($this->session->flashdata('flash_message') == 'updated')
        {
          $msg = "<strong>Well done!</strong> city grade detail updated with success.";
          $style = 'display: block;';
        }else if($this->session->flashdata('flash_message') == 'Deleted')
        {
          $msg = "<strong>Well done!</strong> city grade detail deleted with success.";
          $style = 'display: block;';
        }
        else
        {
    ...
    <div class="block full">
        <div class="block-title">
            <h2><strong>City Grade</strong></h2>
        </div>
        <div class="row">
          <div class="col-md-12">
            <div class="block pull-right no-border">
              <?php echo form_open('admin/cities/add'); ?>
                  <div class="block-section text-right">
                      <button type="submit" class="btn btn-sm btn-primary"><i class="fa fa-plus"></i> Add City Grade</button>
                  </div>
                  <?php echo form_close(); ?>
            </div>     
          </div>  
                ...
                <tbody>
                    ...
                    if($cities){
                         for($i = 0; $i < count($cities); $i++){
                          $id = str_replace('/', '_',rtrim(base64_encode($cities[$i]->id), '=')); 
                    ?>
                        <tr>
                            <td class="text-center"><?php echo str_pad($i + 1, 2,"0", STR_PAD_LEFT);; ?></td>
                            <td class="text-center"><?php echo ucfirst($cities[$i]->country_name); ?></td>
                            <td class="text-center"><?php echo ucfirst($cities[$i]->zone_name); ?></td>
                            <td class="text-center"><?php echo ucfirst($cities[$i]->state_name); ?></td>
                            <td class="text-center"><?php echo ucfirst($cities[$i]->city_name); ?></td>
                            <td class="text-center"><?php echo $cities[$i]->grade; ?></td>
                            <td class="text-center"><?php echo date(DEFAULT_DATETIME_FORMAT,strtotime($cities[$i]->created_at)); ?></td>
                            <td class="text-center">
                                <div class="btn-group">
                                    ...
                                    <a href="<?php echo CURRENT_MODULE.'cities/edit/'.$id;?>" data-toggle="tooltip" title="Edit" class="btn btn-xs btn-default"><i class="fa fa-pencil"></i></a>
