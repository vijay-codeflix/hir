...
Class Notifications_model extends MY_Model
{
  ...
  function delete($id)
  {
   ...
   function getNotificationsList(){
    $this->db->SELECT('not.*, us.first_name, us.last_name,');
    $this->db->FROM('notifications as not');
    $this->db->join('users us', 'us.id =  not.user_id','LEFT');
    $this->db->order_by('created_at', 'desc');    
    $query = $this->db->get();
    if($query->num_rows() > 0){ return $query->result(); }else{ return false;  }
  }

    function getNotificationsRequestList()
    {
      $this->db->SELECT('ofr.*, us.first_name, us.last_name, us.phone');
      $this->db->FROM('notifications_request as ofr');
    ...
    public function getOffDayUser($id)
    {
      ...
      $this->db->FROM('notifications_request');
      $this->db->WHERE('id', $id);
      $query = $this->db->get();
