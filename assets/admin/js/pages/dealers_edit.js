...
function add_new_owner(){
    var clone_owner = $('#owner_contact_details').clone(true);
    // console.log($('#exist_div'));
    
    clone_owner.appendTo('#exist_div');
}

function delete_owner(id){
    var owner_id = $('input[id=owner_id][value='+ id +']');
    owner_id.next('#is_deleted').val('1');
    owner_id.closest('#owner_contact_details').hide();
